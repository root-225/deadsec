...
function SocialMedia() {
    return (<div className="fixed left-0 top-1/2 transform -translate-y-1/2 z-40">
      <div className="flex flex-col space-y-2">
        {socialLinks.map(function (item) { return (<framer_motion_1.motion.a key={item.name} href={item.href} target="_blank" rel="noopener noreferrer" className={"p-3 bg-slate-800/90 backdrop-blur-sm text-white ".concat(item.color, " transition-colors duration-300")} whileHover={{ x: 5 }} whileTap={{ scale: 0.95 }} initial={{ x: -100 }} animate={{ x: 0 }} transition={{
                type: "spring",
                stiffness: 100,
                damping: 20
            }}>
